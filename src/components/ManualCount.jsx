...
import { createManualCount, getAllManualCounts, deleteManualCount } from '../db/indexeddb'
...
function evaluateExpression(input) {
  ...
  try {
    ...
    return { expression: displayExpr, result: Math.round(result) }
  } catch {
    return null
  }
}
...
export default function ManualCount({ onBack }) {
  ...
  // Load session references from Supabase
  useEffect(() => {
    if (!sessionId) return
    setLoadingRefs(true)
  ...
  // Load existing manual counts from IndexedDB
  useEffect(() => {
    if (!sessionId || !userName) return
    getAllManualCounts(sessionId, userName).then(existing => {
      const restored = {}
      existing.forEach(mc => {
        restored[mc.reference] = {
          entries: mc.entries || [],
          total: mc.manualCount || 0,
          localId: mc.id,
        }
      })
      setCounts(restored)
  ...
  function selectReference(ref) {
    ...
    setShowDropdown(false)
    if (!counts[ref]) {
      setCounts(prev => ({ ...prev, [ref]: { entries: [], total: 0, localId: null } }))
    }
  }
  ...
  function addEntry() {
    ...
    setCounts(prev => {
      const current = prev[currentRef] || { entries: [], total: 0, localId: null }
      const newEntries = [...current.entries, evaluated]
      const newTotal = newEntries.reduce((sum, e) => sum + e.result, 0)
      return { ...prev, [currentRef]: { ...current, entries: newEntries, total: newTotal } }
    })
    setInputValue('')
  ...
  function clearReference(ref) {
    const data = counts[ref]
    if (data?.localId) {
      deleteManualCount(data.localId)
    }
    setCounts(prev => {
    ...
    if (currentRef === ref) setCurrentRef(null)
  }

  // Save all counts to IndexedDB
  async function handleSave() {
    const refs = Object.keys(counts).filter(r => counts[r].entries.length > 0)
    for (const ref of refs) {
      const data = counts[ref]
      await createManualCount(ref, data.total, data.entries, sessionId, userName, data.localId)
    }
    setSaved(`${refs.length} reference(s) sauvegardee(s)`)
    // Reload to get localIds
    const existing = await getAllManualCounts(sessionId, userName)
    const restored = {}
    existing.forEach(mc => {
      restored[mc.reference] = {
        entries: mc.entries || [],
  ...
  function appendToInput(char) { setInputValue(prev => prev + char) }
  function backspace() { setInputValue(prev => prev.slice(0, -1)) }
  ...
  return (
    <div className="h-full flex flex-col safe-top safe-bottom bg-slate-50">
      <header className="bg-amber-500 text-white px-4 py-4 flex items-center justify-between">
        <button onClick={onBack} className="p-2 -ml-2 hover:bg-amber-600 rounded-lg transition-colors">
        ...
        <div className="flex items-center gap-1">
          {usedRefs.length > 0 && (
            <button onClick={handleSave} className="p-2 hover:bg-amber-600 rounded-lg transition-colors" title="Sauvegarder">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {saved && <div className="bg-green-500 text-white text-center text-sm py-1.5">{saved}</div>}

      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Reference selection */}
        <div className="p-4 bg-white border-b border-slate-200">
          <label className="block text-sm font-medium text-slate-600 mb-2">Reference en cours</label>
          {currentRef ? (
            <div className="flex items-center gap-2">
              <div className={`flex-1 px-4 py-3 rounded-xl font-bold ${
                sessionRefs.includes(currentRef)
                  ? 'bg-amber-50 border border-amber-200 text-amber-700'
                  : 'bg-purple-50 border border-purple-200 text-purple-700'
              }`}>
                {currentRef}
                {!sessionRefs.includes(currentRef) && (
                  <span className="ml-2 text-xs bg-purple-200 text-purple-800 px-1.5 py-0.5 rounded">hors liste</span>
                )}
              </div>
              <button onClick={() => setCurrentRef(null)} className="p-3 bg-slate-100 rounded-xl hover:bg-slate-200 transition-colors">
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              {showDropdown && (
                <ul className="absolute z-10 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-lg max-h-60 overflow-auto">
                  {filteredRefs.map((ref, idx) => (
                    <li key={idx}>
                      <button onClick={() => selectReference(ref)} className="w-full px-4 py-3 text-left hover:bg-amber-50 transition-colors flex items-center justify-between">
                        <span>{ref}</span>
                  ...
                  {searchNotInSession && (
                    <li>
                      <button onClick={addManualReference} className="w-full px-4 py-3 text-left hover:bg-purple-50 transition-colors border-t border-slate-100 text-purple-700">
              ...
              <p className="text-xs text-slate-400 mt-1">
                {sessionRefs.length} reference(s) disponible(s) — ou saisissez manuellement
              </p>
            </div>
          )}
        </div>
          ...
          <div className="bg-white border-b border-slate-200">
            <div className="bg-slate-800 text-white p-4 mx-4 mt-4 rounded-t-xl">
              <div className="text-right text-2xl font-mono min-h-[2rem] flex items-center justify-end">
                ...
                {preview !== null && <span className="text-slate-400 ml-3">= {preview}</span>}
              </div>
            </div>
            <div className="grid grid-cols-4 gap-1 p-2 mx-4 mb-4 bg-slate-200 rounded-b-xl">
              {[['7','8','9','×'],['4','5','6','−'],['1','2','3','+']].map((row, ri) =>
                row.map((char, ci) => {
                  const isOp = ci === 3
                  const inputChar = char === '×' ? '×' : char === '−' ? '-' : char === '+' ? '+' : char
                  return (
                    <button key={`${ri}-${ci}`} onClick={() => appendToInput(inputChar)}
                      className={`${isOp ? 'bg-amber-500 hover:bg-amber-600 text-white' : 'bg-white hover:bg-slate-50 text-slate-800'} text-2xl font-bold py-4 rounded-lg active:scale-95 transition-all`}>
                      {char}
                    </button>
                  )
                })
          ...
          {currentRef && counts[currentRef] && counts[currentRef].entries.length > 0 ? (
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
              <div className="px-4 py-3 bg-amber-50 border-b border-amber-100 flex items-center justify-between">
                <span className="font-bold text-amber-700">Detail des entrees</span>
              ...
              <div className="divide-y divide-slate-100">
                {counts[currentRef].entries.map((entry, idx) => (
                  <div key={idx} className="px-4 py-3 flex items-center justify-between">
                    ...
                    <button onClick={() => removeEntry(idx)} className="p-2 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
            ...
            <div className="text-center py-12 text-slate-400">
              ...
              <p>Commencez a entrer des nombres</p>
            </div>
          ) : (
              ...
              {usedRefs.length > 0 ? (
                <>
                  <h3 className="text-sm font-medium text-slate-500 uppercase tracking-wide">
                    References comptees ({usedRefs.length})
                  </h3>
                  {usedRefs.map(ref => (
                    <div key={ref} className="bg-white rounded-xl p-4 shadow-sm">
                      <div className="flex items-center justify-between mb-2">
                        <button onClick={() => selectReference(ref)} className="font-bold text-slate-800 hover:text-amber-600 transition-colors">
                          {ref}
                          {!sessionRefs.includes(ref) && (
                            <span className="ml-2 text-xs bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded">hors liste</span>
                          )}
                        </button>
                        <div className="flex items-center gap-2">
                          <span className="text-xl font-bold text-amber-600">{counts[ref].total}</span>
                          <button onClick={() => clearReference(ref)} className="p-1 text-red-400 hover:text-red-600 transition-colors">
                ...
                <div className="text-center py-12 text-slate-400">
                  ...
                  <p>Selectionnez une reference pour commencer</p>
                </div>
              )}
            </div>
          )}
